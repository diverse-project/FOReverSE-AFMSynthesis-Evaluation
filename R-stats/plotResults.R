...
scalF <- scalF[,c("#variables", "Synthesis")] 
scalF <- data.frame(scalF[1], lapply(scalF[2], function(x) (x/1000)**(1/2.5)), check.names = FALSE)
...
abline(scalF.lm)
print(cor(scalF$"Synthesis", scalF$"#variables"))
...
scalC <- data.frame(scalC[1], lapply(scalC[2], function(x) (x/1000)**(1/1.5)), scalC[3], check.names = FALSE)
...
abline(scalC.lm)
print(cor(scalC$"Synthesis", scalC$"#distinct configurations"))
...
abline(scalD.lm)
print(cor(scalD$"Synthesis", scalD$"real max domain size"))
...
scalFor <- data.frame(scalFor[1], lapply(scalFor[2], function(x) (x/60000)**(1/2)), check.names = FALSE)
...
# Bar plots
filteredTimes <- results[results$"#variables" == 10 & results$"#configurations" == 10000 & results$"enable or groups" == 'false',]
